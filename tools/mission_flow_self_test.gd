...
func _test_overtime() -> void:
	...
	if main_file != null:
		...
		_expect(source.contains("MAX_AMBIENT_CONTACTS := 6") and source.contains("enemies.size() < MAX_AMBIENT_CONTACTS"), "ambient spawns should leave airspace capacity for authored formations, warnings and readable attack passes")
...
func _test_pixel_ui() -> void:
	...
	if source_file != null:
		...
		_expect(source.contains('var cue := " OPEN" if phase >= 3'), "phase-three boss HUD should retain exposed-mechanism cue")
		...
		_expect(source.contains("func _hud_housing_alpha") and source.contains("--capture-hud-clearance") and source.contains("return 0.48") and source.contains('["enemies","protected_contacts","enemy_bullets"]'), "lower corner HUD housings should yield to live aircraft, missiles, objectives and protected contacts without hiding symbology")
