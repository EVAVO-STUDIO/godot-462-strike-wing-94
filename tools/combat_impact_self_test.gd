...
func _initialize() -> void:
	...
	var intercept_velocity := ProjectileRules.enemy_intercept_velocity(Vector2(200,100),Vector2(200,260),Vector2(80,0),200.0)
	check(intercept_velocity.x > 0.0 and intercept_velocity.y > 0.0 and is_equal_approx(intercept_velocity.length(),200.0), "traversing mounts should lead a laterally moving aircraft at authored projectile speed")
	var capped_lead := ProjectileRules.enemy_intercept_velocity(Vector2(200,100),Vector2(200,500),Vector2(200,0),100.0,0.5)
	check(capped_lead.normalized().x < 0.25, "turret prediction should remain bounded instead of producing implausible extreme lead")
	...
	check(ProjectileRules.hostile_fire_authorized(ProjectileRules.MAX_ACTIVE_HOSTILE_PROJECTILES - 1), "ordinary batteries should fire below the shared hostile-projectile ceiling")
	check(not ProjectileRules.hostile_fire_authorized(ProjectileRules.MAX_ACTIVE_HOSTILE_PROJECTILES), "ordinary batteries should hold fire when the combat picture is saturated")
	check(ProjectileRules.hostile_fire_authorized(ProjectileRules.MAX_ACTIVE_HOSTILE_PROJECTILES, true), "boss batteries should retain a small projectile reserve for phase identity")
	check(not ProjectileRules.hostile_fire_authorized(ProjectileRules.MAX_ACTIVE_HOSTILE_PROJECTILES + ProjectileRules.BOSS_PROJECTILE_RESERVE, true), "boss batteries should still respect the absolute readability ceiling")
