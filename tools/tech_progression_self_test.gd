...
func _test_era_order() -> void:
	...
	_expect(not TechProgressionRules.can_unlock("invalid_required", "strategic_orbital"), "malformed required era must fail closed")
...
func _test_weapon_gates() -> void:
	...
	var plasma := _item_for_id(weapons, "plasma_lance")
	...
	_expect(str(storm.get("archetype", "")) == "directed_energy_pulse", "Storm Cannon should retain bounded pulse identity")
	_expect(str(plasma.get("unlock_tech_era", "")) == "strategic_orbital", "Plasma Lance should unlock only in the strategic-orbital era")
	_expect(str(plasma.get("archetype", "")) == "strategic_plasma", "Plasma Lance should retain strategic plasma identity")
	_expect(int(plasma.get("projectiles", 0)) == 1 and int(plasma.get("damage", 0)) >= 8 and float(plasma.get("energy_cost", 0)) >= 28.0, "Plasma Lance should be a single heavy high-demand primary")
	...
	_expect(TechProgressionRules.can_unlock(str(storm.get("unlock_tech_era", "")), "directed_energy"), "directed-energy era should unlock Storm Cannon")
	_expect(not TechProgressionRules.can_unlock(str(plasma.get("unlock_tech_era", "")), "directed_energy"), "Plasma Lance should remain locked throughout Missions 9-11")
	_expect(TechProgressionRules.can_unlock(str(plasma.get("unlock_tech_era", "")), "strategic_orbital"), "Mission 12 strategic-orbital era should unlock Plasma Lance")
...
func _test_generator_gates() -> void:
	...
	_expect(float(overdrive.get("efficiency_multiplier", 1.0)) < float(pulse.get("efficiency_multiplier", 1.0)), "Overdrive Core should provide stronger late-tech efficiency")
	...
	var plasma := {"energy_cost":28.0,"unlock_tech_era":"strategic_orbital"}
	...
	_expect(EnergyRules.effective_weapon_cost(plasma, overdrive) < EnergyRules.weapon_cost(plasma), "Overdrive Core should remain capable of powering later strategic-orbital hardware efficiently")
...
func _test_emp_resistance() -> void:
	...
	var ark := _item_for_id(enemies, "machine_ark")
	var scout_res := SupportRules.emp_resistance(scout)
	var sentry_res := SupportRules.emp_resistance(sentry)
	var ark_res := SupportRules.emp_resistance(ark)
	_expect(scout_res < sentry_res and sentry_res < ark_res, "autonomous EMP resistance should escalate through the machine-war endgame")
	_expect(SupportRules.emp_effective_duration(3.0, scout_res) > SupportRules.emp_effective_duration(3.0, ark_res), "higher EMP resistance should shorten disruption")
	_expect(SupportRules.emp_speed_scale(scout_res) < SupportRules.emp_speed_scale(ark_res), "higher EMP resistance should retain more movement")
