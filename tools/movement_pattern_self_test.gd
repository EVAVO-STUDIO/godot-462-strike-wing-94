...
func _initialize() -> void:
	...
	var collision_unload := MovementPatternRules.adjusted_motion("sine_dive",Vector2(394,132),player,2.0,0.1,180.0,0.0)
	_expect(float(collision_unload.desired_lateral_velocity)<-90.0, "an attack pass entering the player's collision corridor should commit away before physical overlap")
	...
	var ship_helm := MovementPatternRules.adjusted_motion("water_lane",Vector2(200,100),player,4.0,0.1,200.0,0.0)
	var ship_settled := MovementPatternRules.adjusted_motion("water_lane",Vector2(228,100),player,8.0,0.1,200.0,0.0)
	_expect(float(ship_helm.desired_lateral_velocity) > 0.0 and is_zero_approx(float(ship_settled.desired_lateral_velocity)), "ships should answer one helm command and settle on a parallel course instead of perpetually wagging")
	...
	var telemetry_source := FileAccess.get_file_as_string("res://tools/playtest_telemetry_probe.gd")
	var steering_index := telemetry_source.find("_drive_movement(whole_second)")
	var cadence_index := telemetry_source.find("if whole_second != last_second:")
	_expect(steering_index >= 0 and cadence_index >= 0 and steering_index < cadence_index, "bounded playtest steering should react every frame while command pulses remain on the one-second schedule")
	_expect(telemetry_source.contains("func _drive_live_countermeasure_response") and telemetry_source.contains('projectile.get("homing", false)') and telemetry_source.contains('projectile.get("weapon_id", "")') and telemetry_source.contains("_defensive_response_cooldown = 1.15"), "bounded vulnerable telemetry should answer live homing missile threats instead of waiting for a blind fixed-second flare schedule")
	var balance_runner := FileAccess.get_file_as_string("res://tools/run_vulnerable_balance_telemetry.ps1")
	_expect(balance_runner.contains("for ($Attempt = 1; $Attempt -le 3; $Attempt++)") and balance_runner.contains("$TransientShutdown"), "vulnerable balance evidence should retry bounded transient Godot shutdown faults")
