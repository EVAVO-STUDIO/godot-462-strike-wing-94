...
func _initialize() -> void:
	...
	_expect(source.contains('boss.get("boss_phase", BossRules.phase_for') and source.contains('cue := " OPEN" if phase >= 3'), "boss HUD should expose authoritative phase and critical weak-point state")
	_expect(source.contains('replace("_", " ")') and source.contains('P%d%s  %d%%'), "boss HUD should show readable identity, phase, and compact integrity percentage")
	_expect(source.contains("Rect2(168,42,304,22)") and source.contains("272.0*ratio"), "boss HUD should remain a compact command strip outside the central combat lane")
