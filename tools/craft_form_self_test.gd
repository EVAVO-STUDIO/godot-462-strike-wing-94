...
func _test_forms() -> void:
	...
	_expect(is_equal_approx(CraftFormRules.TRANSFORM_WEAPON_INTERLOCK, CraftFormRules.TRANSFORM_VISUAL_SECONDS) and CraftFormRules.TRANSFORM_WEAPON_INTERLOCK < CraftFormRules.TRANSFORM_COOLDOWN, "weapon hardpoints should remain interlocked through the complete visible sweep while the anti-spam cooldown remains slightly longer")
	_expect(CraftFormRules.TRANSFORM_VISUAL_SECONDS >= 1.10 and CraftFormRules.TRANSFORM_VISUAL_SECONDS <= 1.25, "canonical wing sweep should hold ten readable near-one-second exposures")
	...
	director.set("_transform_from_form", CraftFormRules.FIGHTER)
	director.set("form", CraftFormRules.BOMBER)
	_expect(is_equal_approx(float(director.call("form_blend_ratio")), 0.0), "fighter-to-bomber mechanics should begin from fighter geometry instead of snapping to the destination form")
	var fighter_collision := CraftFormRules.collision_radius_sq(CraftFormRules.FIGHTER)
	var bomber_collision := CraftFormRules.collision_radius_sq(CraftFormRules.BOMBER)
	var half_sweep := CraftFormRules.TRANSFORM_VISUAL_SECONDS * 0.50
	director.call("_update_transform_settle", fixture, half_sweep)
	...
	var mid_blend := float(director.call("form_blend_ratio"))
	var mid_collision := float(director.call("collision_radius_sq"))
	_expect(mid_blend > 0.45 and mid_blend < 0.60 and mid_collision > fighter_collision and mid_collision < bomber_collision, "collision geometry should expand progressively with the visible wing sweep")
	director.call("_update_transform_settle", fixture, half_sweep + 0.02)
	...
	_expect(is_equal_approx(float(director.call("form_blend_ratio")), 1.0) and is_equal_approx(float(director.call("collision_radius_sq")), bomber_collision), "settled bomber mechanics should match the completed authored geometry")
...
func _test_source_integration() -> void:
	...
	if director_file != null:
		...
		_expect(source.contains("func form_blend_ratio") and source.contains("_blended_form_value") and source.contains("altitude_transition_active() or transform_active()"), "live form mechanics should track the authored sweep and block hypersonic charge until geometry settles")
	...
	_expect(transition_source.contains("ATMOSPHERIC_VEIL") and transition_source.contains("for i in range(7)") and transition_source.contains("var travel := 414.0") and transition_source.contains("CLOUD_SHADOW") and transition_source.contains("_draw_layer_exposure") and transition_source.contains("_draw_depth_rush"), "altitude transitions should cross a registered cloud boundary with directional depth travel and authored atmospheric layering")
