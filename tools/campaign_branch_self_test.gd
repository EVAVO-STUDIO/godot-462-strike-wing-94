...
func _run() -> void:
	...
	_expect(int(_json("res://data/campaign.json").get("save", {}).get("schema_version", 0)) == 9, "campaign metadata should advertise canonical save schema v9")
	...
	_expect(save.contains('SAVE_VERSION := 9') and save.contains('"branch_decisions"'), "branch decisions should persist in save schema v9")
