...
func _initialize() -> void:
	...
	_expect(FlightSpeedRules.contact_screen_speed(1.0,"ground",0.0)>0.0, "static surface sites must travel with geographic route motion")
	_expect(FlightSpeedRules.contact_screen_speed(4.4,"ground",0.0)>FlightSpeedRules.contact_screen_speed(1.0,"ground",0.0)*4.0, "hypersonic travel must accelerate static terrain contacts with the map")
	_expect(FlightSpeedRules.contact_screen_speed(1.0,"sea",68.0)>FlightSpeedRules.contact_screen_speed(1.0,"sea",34.0), "ship propulsion should remain visible on top of shared water closure")
	_expect(FlightSpeedRules.contact_screen_speed(1.0,"ground",24.0)-FlightSpeedRules.contact_screen_speed(1.0,"ground",0.0)<5.0, "road speed should not detach vehicles from their terrain")
	...
	_expect(main_source.contains("FlightSpeedRules.contact_screen_speed") and main_source.contains("FlightSpeedRules.recovery_closure_multiplier"), "forward power should accelerate terrain-fixed contacts and recovery pods with the world")
