...
func _initialize() -> void:
	...
	_expect(settings.call("setting_value",4)=="075%","SFX option should expose exact output level")
	_expect(settings.call("setting_value",5)=="18%","controller option should expose exact deadzone")
	_expect(settings.call("setting_value",8)=="AUTHENTIC","projectile contrast should default to authentic presentation")
	_expect(settings.call("setting_label",9)=="CAMPAIGN DIFFICULTY" and settings.call("setting_value",9)=="COMBAT","options should expose persistent campaign difficulty")
	_expect(int(settings.call("setting_count"))==10 and int(settings.call("category_count"))==5,"front-end and tactical-pause options should share complete category metadata")
	_expect(settings.call("category_name",3)=="ACCESS" and int(settings.call("category_global_index",3,2))==8,"accessibility page should map its rows canonically")
	_expect(absf(float(settings.call("setting_ratio",4))-0.75)<0.001,"SFX meter ratio should match saved level")
