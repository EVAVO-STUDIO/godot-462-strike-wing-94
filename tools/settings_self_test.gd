...
func _initialize() -> void:
	...
	_expect(settings.call("setting_label",11)=="CAMPAIGN DIFFICULTY" and settings.call("setting_value",11)=="COMBAT","options should expose persistent campaign difficulty")
	_expect(int(settings.call("setting_count"))==12 and int(settings.call("category_count"))==5,"front-end and tactical-pause options should share complete category metadata")
	_expect(settings.call("category_name",3)=="ACCESS" and int(settings.call("category_global_index",3,2))==10,"accessibility page should map its rows canonically")
