...
$Source = Join-Path $Root 'assets/source/enemies/mercenary_machine_phase_v2'
...
$Run = 'work/mercenary_machine_phase_build/' + [guid]::NewGuid().ToString('N')
...
if ($LASTEXITCODE -ne 0) { throw 'EVAVO boss phase-art plan compilation failed.' }
& $PythonBin (Join-Path $ArtStudioRoot 'tools/run_project_art_sandbox.py') --workspace-root $Root --plan $Plan --output-root $Candidate
if ($LASTEXITCODE -ne 0) { throw 'EVAVO boss phase-art sandbox execution failed.' }
...
foreach ($Frame in $Manifest.frames) {
    ...
    if ((Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $Frame.sha256) {
        throw "Boss phase-art rebuild differs from reviewed pixels: $($Frame.file)"
    }
}
...
Write-Host 'Rebuilt 30 reviewed mercenary and machine boss phase overlays through EVAVO Art Studio.'
