...
func _test_production_art() -> void:
	...
	_expect(explosion_builder.contains("flash-pressure-fireball-smoke-fragment") and explosion_builder.contains("ground-coupled-dirt-flame-column-fragment") and explosion_builder.contains("directional-hot-metal-spall") and explosion_builder.contains("def pressure_arc") and not explosion_builder.contains("draw.arc"), "weapon explosion builder should preserve distinct missile, ground-blast and cannon motion languages without geometric pressure rings")
	_expect(explosion_builder.contains("rng = random.Random(18400)") and explosion_builder.contains("rng = random.Random(29400)") and not explosion_builder.contains("random.Random(18400 + index)") and not explosion_builder.contains("random.Random(29400 + index)"), "rocket and cannon material silhouettes should remain registered between held-cel exposures")
	...
	_expect(FileAccess.file_exists("res://assets/source/support/aircraft_v3/hammer_bomber_manifest.json"), "Hammer v3 should retain its native-scale production manifest")
...
func _test_source_contract() -> void:
	...
	if file != null:
		...
		_expect(source.contains('strike_cel("gunship_impact"') and not source.contains('staged_effect("impact", (burst_phase'), "Spectre fire mission should use directional cannon debris instead of the generic star impact")
		_expect(source.contains("attack_curve := smoothstep") and source.contains("roll_in :=") and source.contains("roll_out :="), "Rapier sweep should fly a banked intercept and breakaway rather than slide on a straight rail")
		_expect(source.contains("release_time :=") and source.contains("fall * fall * 118.0") and source.contains("egress := smoothstep"), "Hammer strike should separate ballistic bomb fall from its banked egress path")
		_expect(source.contains("orbit_angle :=") and source.contains("154.0 * cos(orbit_angle)") and source.contains("pylon turn"), "Spectre should cross the target on a readable banked pylon turn rather than hover")
		_expect(source.contains("intercept_ratio :=") and source.contains('frame_for_clock("precision_missile"') and source.contains('strike_cel("missile_impact",burst_ratio)'), "Rapier sweep should visibly launch and resolve a guided air-to-air missile")
		...
		var capture_fixtures_present := source.contains('argument.begins_with("--capture-support=")')
		for fixture in ['"tanker"', '"fighter"', '"bomber"', '"gunship"', '"missile"', '"missile_impact"', '"rail"', '"orbital"']:
			capture_fixtures_present = capture_fixtures_present and source.contains(fixture)
		_expect(capture_fixtures_present, "all battlefield support presentations should expose deterministic visual QA fixtures")
		_expect(source.contains('"fighter_impact": _draw_fighter_sweep(surface, 0.58)'), "Rapier airburst should expose a deterministic visual QA fixture")
		_expect(source.contains('"fighter_breakaway": _draw_fighter_sweep(surface, 0.82)') and source.contains('"bomber_release": _draw_bomber_run(surface, 0.43)') and source.contains('"bomber_egress": _draw_bomber_run(surface, 0.82)'), "aircraft support passes should expose deterministic attack and egress evidence")
		_expect(source.contains('"gunship_ingress": _draw_gunship_fire(surface, 0.18)') and source.contains('"gunship_egress": _draw_gunship_fire(surface, 0.88)'), "Spectre pylon turn should expose deterministic ingress and egress evidence")
