...
func _initialize() -> void:
	...
	if guidance_script != null:
		...
		_expect(str(guidance.call("_forced_guidance", "steer_fire").get("text", "")).contains("STEER A-D/LS") and str(guidance.call("_forced_guidance", "steer_fire").get("text", "")).contains("FIRE SPACE/A"), "Mission 1 should teach steering and primary fire without a modal tutorial")
		_expect(str(guidance.call("_forced_guidance", "power_geometry").get("text", "")).contains("GEOMETRY Q/Y"), "Mission 1 should introduce throttle and VX-94 geometry")
		...
		_expect(str(guidance.call("_forced_guidance", "countermeasure").get("text", "")).contains("FLARE V/LT"), "first real missile threat should expose countermeasure input")
