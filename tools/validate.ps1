...
$Required = @(
    'project.godot','export_presets.cfg','tools/export_windows.ps1','tools/verify_windows_export.ps1','tools/validate_windows_release.ps1','tools/run_performance_profile.ps1','tools/performance_probe.gd','tools/run_visual_qa.ps1','tools/visual_capture_probe.gd','tools/run_playtest_telemetry.ps1','tools/playtest_telemetry_probe.gd','tools/build_application_icon.ps1','assets/runtime/brand/hypersonic_application_icon.png','scenes/main.tscn','scripts/main.gd','scripts/content_catalog.gd','scripts/product_identity.gd','scripts/input_bindings.gd','scripts/settings_director.gd','scripts/startup_sequence_director.gd','scripts/startup_sequence_surface.gd','data/product_identity.json',
    ...
    'scripts/mission_intel_rules.gd','scripts/mission_intel_surface.gd','scripts/mission_intel_director.gd','scripts/mission_radio_surface.gd','scripts/mission_radio_director.gd',
    ...
    'tools/strike_ordnance_self_test.gd','tools/tech_progression_self_test.gd','tools/boss_hud_self_test.gd','tools/boss_signature_self_test.gd','tools/combat_art_self_test.gd','tools/retro_sfx_self_test.gd','tools/afterburner_self_test.gd','tools/hypersonic_self_test.gd','tools/player_mount_self_test.gd',
...
if (@($Missions.missions).Count -ne 30) { throw 'Core campaign requires exactly thirty authored missions.' }
...
if ($CampaignIds.Count -ne 30 -or @($CampaignIds | Sort-Object -Unique).Count -ne 30) { throw 'Campaign order must contain thirty unique missions.' }
...
foreach ($Autoload in @(
    ...
    'MissionRadioDirector="*res://scripts/mission_radio_director.gd"',
...
$ReleaseValidationScriptPath = Join-Path $Root 'tools/validate_windows_release.ps1'
$ReleaseValidationTokens = $null
$ReleaseValidationErrors = $null
[System.Management.Automation.Language.Parser]::ParseFile($ReleaseValidationScriptPath, [ref]$ReleaseValidationTokens, [ref]$ReleaseValidationErrors) | Out-Null
if ($ReleaseValidationErrors.Count -gt 0) { throw "Windows release validation script has PowerShell parser errors: $($ReleaseValidationErrors[0].Message)" }
$PerformanceScriptPath = Join-Path $Root 'tools/run_performance_profile.ps1'
...
[System.Management.Automation.Language.Parser]::ParseFile($PlaytestTelemetryScriptPath, [ref]$PlaytestTelemetryTokens, [ref]$PlaytestTelemetryErrors) | Out-Null
if ($PlaytestTelemetryErrors.Count -gt 0) { throw "Playtest telemetry script has PowerShell parser errors: $($PlaytestTelemetryErrors[0].Message)" }
...
$Tests = @(
    ...
    'strike_ordnance_self_test.gd','tech_progression_self_test.gd','boss_hud_self_test.gd','boss_signature_self_test.gd','combat_art_self_test.gd','retro_sfx_self_test.gd','afterburner_self_test.gd','hypersonic_self_test.gd','player_mount_self_test.gd'
)
