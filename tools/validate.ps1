...
$Required = @(
    ...
    'tools/product_identity_self_test.gd','tools/input_bindings_self_test.gd','tools/startup_sequence_self_test.gd','tools/runtime_self_test.gd','tools/reward_self_test.gd','tools/service_self_test.gd','tools/mission_flow_self_test.gd','tools/save_recovery_self_test.gd',
...
if (@($Missions.missions).Count -ne 24) { throw 'Campaign tranche requires twenty-four authored missions.' }
...
if ($CampaignIds.Count -ne 24 -or @($CampaignIds | Sort-Object -Unique).Count -ne 24) { throw 'Campaign order must contain twenty-four unique missions.' }
