...
func _test_visual_language() -> void:
	...
	if typeof(surface_site_manifest) == TYPE_DICTIONARY:
		_expect(int(surface_site_manifest.get("schema_version", 0)) >= 6 and str(surface_site_manifest.get("perspective", "")).contains("oblique orthographic"), "surface-site family should use the registered cel-art perspective authority")
		...
		for site_id in surface_site_manifest.get("military", []) + surface_site_manifest.get("protected", []):
			var runtime_name := "civilian_village_v2" if str(site_id) == "civilian_village" else str(site_id)
			var site_texture := load("res://assets/runtime/surface_sites/%s.png" % runtime_name)
	...
	for animation_path in ["radar_site/0.png","radar_site/1.png","radar_site/2.png","radar_site/3.png","field_artillery/0.png","field_artillery/1.png","ballistic_launcher/0.png","ballistic_launcher/1.png","ballistic_launcher/2.png","damage/0.png","damage/1.png"]:
	...
	_expect(source.contains("Vector2(scale*0.94,scale*0.42)") and source.contains("Color(0.025,0.03,0.028,0.32)"), "surface-site silhouettes should cast a registered terrain-contact exposure")
	_expect(source.contains('enemy_id == "ballistic_launcher"') and source.contains("deployment_frame"), "mobile ballistic launcher should visibly raise from travel to firing posture")
	_expect(source.contains("SURFACE_SITE_DAMAGE") and source.contains("integrity <= 0.62") and source.contains("integrity <= 0.32"), "damaged surface sites should expose two authored structural-loss thresholds")
	...
	_expect(source.contains("func _draw_infantry_member") and source.contains("scale *= 1.24") and source.contains("shadow_size") and not source.contains("Color(1.10,1.08,1.02,0.20)"), "human-scale squads should retain a compact optical-scale allowance and registered contact shadows without a luminous duplicate sprite pass")
	...
	_expect(source.contains("func _render_airframe_maneuver_vapor") and source.contains('enemy_id in ["scout_falcon", "ace_interceptor"]') and source.contains('FRAMES["contrail"]') and source.contains("bank_load * speed_load"), "fast conventional interceptors should pull short authored wingtip vapor only under real speed and bank load")
	...
	var machine_scout := load("res://assets/runtime/enemies/machine_air/drone_scout_idle.png") as Texture2D
	_expect(machine_scout != null and machine_scout.get_image().get_used_rect().size.x >= 20, "machine scout should retain a broad aircraft planform instead of a humanoid vertical silhouette")
	_expect(FileAccess.file_exists("res://assets/source/enemies/machine_air_scout_v4/manifest.json"), "machine scout level and bank cels should retain their authored v4 source manifest")
...
func _test_altitude_presentation() -> void:
	...
	_expect(source.contains("22.0 * float(direction)") and source.contains("longitudinal_scale") and source.contains("Vector2(depth_scale,depth_scale*longitudinal_scale)"), "VX-94 climb and dive pitch should combine centre-of-mass travel with longitudinal foreshortening at native gameplay scale")
...
func _test_combat_fx() -> void:
	...
	_expect(source.contains("A killing cannon burst ruptures the target") and source.contains("fireball_size := roundf(max_size*1.35)") and source.contains("rupture_alpha") and source.contains("ground_fireball_size") and source.contains("Surface warheads need a dense incandescent core") and source.contains("MISSILE_BLAST_SCALE := 4.15") and source.contains("SURFACE_BLAST_SCALE := 3.65") and source.contains("CANNON_KILL_SCALE := 2.15") and source.contains("pale condensation") and source.contains("detonation_grade"), "lethal cannon, rocket and missile impacts should preserve a believable scale hierarchy, spark-led cannon rupture, dense warhead cores and a restrained condensation front")
	_expect(source.contains("func _draw_blast_volume") and source.contains("Start the retained smoke while the incandescent cel is still expanding") and source.contains('impact_family in ["missile", "rocket", "bomb", "cannon"]'), "lethal impacts should overlap hot cels with retained smoke volume and throw fragments from cannon kills as well as warheads")
	_expect(source.contains("fragment_seed") and source.contains("2.39996323") and source.contains("vertical = -0.18-absf(vertical)*0.88+travel*0.72") and not source.contains("float(index)/float(count)*TAU"), "hot fragment throws should be deterministic and asymmetric, with surface ejecta biased above the impact plane instead of forming arcade starbursts")
	...
	_expect(source.contains("var draw_dimensions") and source.contains("draw_size*1.20,draw_size*0.82") and source.contains("draw_size*0.88,draw_size*1.12"), "missile airbursts, ground-coupled warheads and directional cannon spall should retain distinct physical blast planes")
