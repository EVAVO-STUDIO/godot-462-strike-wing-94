...
func _test_retro_sfx() -> void:
	...
	if cue_file != null:
		...
		_expect(cue_source.contains("_meter_reveal_timer") and cue_source.contains("absf(throttle-_last_throttle)") and cue_source.contains("or ratio <= 0.20"), "large propulsion instrument should auto-hide in steady cruise and return for power changes or critical states")
