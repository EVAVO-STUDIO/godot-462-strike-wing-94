...
if ([int]$Handoff.required_journey_count -ne 10) { throw 'Native Test Lab handoff must contain all 10 governed release journeys.' }
...
if (-not [bool]$Handoff.controller_menu_required) { throw 'Native Test Lab handoff lost the required controller Options/Flight-Controls journey.' }
if ([string]$Handoff.profile -ne '.evavo/godot-lab-native.json') { throw 'Native Test Lab handoff core profile is incorrect.' }
if ([string]$Handoff.controller_sortie_profile -ne '.evavo/godot-lab-controller-sortie.json') { throw 'Native Test Lab handoff controller front-end profile is incorrect.' }
...
Write-Host "HYPERSONIC native handoff passed: 10 journeys including controller maintenance and menu navigation, target $HeadSha, Lab $ExpectedLabSha." -ForegroundColor Green
