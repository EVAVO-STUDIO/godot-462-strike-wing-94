...
func run() -> void:
	...
	var acceleration_step := absf(Camera.advance_offset(0.0,4.4,0.1))
	var recovery_step := absf(Camera.advance_offset(Camera.target_offset(4.4),1.0,0.1)-Camera.target_offset(4.4))
	check(acceleration_step > recovery_step*1.5, "Hypersonic power should punch into look-ahead faster than the airframe settles back to cruise")
