...
func _initialize() -> void:
	...
	_expect(FileAccess.file_exists("res://assets/source/effects/countermeasure_v3/manifest.json"), "paired magnesium flare cels should retain their deterministic v3 source manifest")
	for index in range(1,4):
		var flare_image := (load("res://assets/runtime/effects/countermeasure/flare_%d.png" % index) as Texture2D).get_image()
		_expect(flare_image.get_used_rect().size.y >= 24, "flare thermal tail and smoke wake should remain readable at native scale: %d" % index)
	...
	_expect(director_source.contains("[-0.78,0.64,-0.42,0.31,-0.12]") and director_source.contains("[1.08,0.78,0.94,0.69,0.86]") and director_source.contains("[0.0,0.070,0.150,0.240,0.340]") and director_source.contains("Vector2(0.60,0.60)"), "countermeasure cassette should ripple unequal port and starboard impulses into a layered ten-decoy break with readable white-hot cartridges")
