...
func _initialize() -> void:
	...
	_expect(director_source.contains("[-0.72,0.72,-0.48,0.48,0.0]") and director_source.contains("[-14.0,14.0,-10.0,10.0,0.0]") and director_source.contains("[0.0,0.055,0.110,0.165,0.220]") and director_source.contains("Vector2(0.58,0.58)"), "countermeasure cassette should alternate port and starboard into a staggered ten-decoy fan rather than resemble an engine fire")
	_expect(director_source.contains("puff_alpha*1.6") and director_source.contains("ignition*0.72"), "flare smoke and ignition should remain subordinate to individually readable decoy heads")
	...
	_expect(director_source.contains("position += trail_direction*ratio*CountermeasureRules.DECOY_TRAIL_DISTANCE") and not director_source.contains("sin(ratio * PI)"), "flare cartridges and smoke should follow one bank-aware ballistic ejection vector")
