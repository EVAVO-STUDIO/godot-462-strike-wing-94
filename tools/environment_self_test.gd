...
func _initialize() -> void:
	...
	if director_file != null:
		...
		var modular_coast_builder := FileAccess.get_file_as_string("res://tools/build_modular_coast_art.ps1")
		_expect(modular_coast_builder.contains("a*max(0,(r+g+b)/3-0.34)/0.66"), "breakwater impacts should remove the source-water plate and retain transparent foam")
		...
		_expect(not source.contains("COAST_BREAKWATER_IMPACT") and not source.contains("impact_y :="), "finite breakwater impacts must not loop as ambient offshore bubbles; route breaker phases own continuous surf")
		...
		_expect(source.contains("COAST_ROUTE_SCROLL_SCALE := 0.82") and source.count("COAST_ROUTE_SCROLL_SCALE") >= 3, "opening coast geography, breakers and registered modules should traverse multiple authored districts at playable speed")
		...
		_expect(source.contains("vertical travel") and source.contains("Vector2(streak_width,streak_height)"), "hypersonic exposure streaks should follow forward screen travel rather than smear sideways")
		...
		_expect(source.contains("Color(0.82,0.88,0.91,0.11*mix)") and source.contains("range(3)") and source.contains("round(2.0*mix)"), "high altitude should open above the main cloud deck with sparse cirrus and shear banks")
