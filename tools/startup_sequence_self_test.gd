...
func _initialize() -> void:
	...
	if startup != null:
		...
		_expect(float(startup.BLACK_PAUSE_SECONDS) >= 0.25, "publisher and title sequences should retain the shared ident's perceptible black pause", failures)
	...
	_expect(wordmark is Texture2D and wordmark.get_size() == Vector2(500,80), "HYPERSONIC wordmark should retain reviewed runtime geometry", failures)
	...
	if manifest_file != null:
		...
		_expect(typeof(manifest) == TYPE_DICTIONARY and str(manifest.get("status", "")) == "runtime_master_v3_approved", "HYPERSONIC wordmark should remain an approved production master", failures)
