...
func review() -> void:
	var low_cloud:Texture2D=load("res://assets/runtime/environments/clouds/cloud_bank_low_wisp_a.png")
	assert(low_cloud.get_size()==Vector2(192,64),"New low-cloud import must be loaded")
	...
	var out:="res://work/low_cloud_v3/native_verified/"
	...
	f.store_string(JSON.stringify({"scope":"Actual production cloud renderer with one refined low-altitude wisp, firing, throttle and afterburner input","frames":evidence},"\t"))
	f.close()
