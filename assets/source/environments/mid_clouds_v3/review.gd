...
func review() -> void:
	for id in ["a","b","c","d"]:
		var cloud:Texture2D=load("res://assets/runtime/environments/clouds/cloud_bank_mid_broken_%s.png" % id)
		assert(cloud.get_size()==Vector2(256,112),"Refined mid cloud must be imported")
	...
	var out:="res://work/mid_clouds_v3/native_review/"
	...
	f.store_string(JSON.stringify({"scope":"Actual production cloud renderer with four refined mid-altitude banks, firing, throttle and afterburner input","frames":evidence},"\t"))
	f.close()
