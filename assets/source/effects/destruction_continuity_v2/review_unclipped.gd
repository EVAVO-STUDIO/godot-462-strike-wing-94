...
class ReviewSurface extends Node2D:
	...
	var cases: Array
	func _draw() -> void:
		draw_rect(Rect2(0,0,640,360), Color("20292c"))
		for row in 2:
			var fx: Node = baseline if row == 0 else candidate
			var cy := 102.0 + row * 166.0
			draw_string(ThemeDB.fallback_font, Vector2(8,17+row*166), ("CURRENT" if row==0 else "RETAINED HULL") + "  %.3fs" % elapsed,HORIZONTAL_ALIGNMENT_LEFT,-1,12,Color.WHITE)
			for i in cases.size():
				var c: Array = cases[i]
				var p := Vector2(108+i*212,cy)
				draw_string(ThemeDB.fallback_font,p+Vector2(-96,68),str(c[0]),HORIZONTAL_ALIGNMENT_LEFT,-1,11,Color.WHITE)
...
func review() -> void:
	...
	surface.baseline=load("res://work/destruction_continuity_v2/baseline.gd").new()
	surface.candidate=load("res://work/destruction_continuity_v2/candidate.gd").new()
	layer.add_child(surface)
	for group in groups.size():
		surface.cases=groups[group]
		var output := "res://work/destruction_continuity_v2/group_%d/" % group
		DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(output))
		for frame in 73:
			surface.elapsed=float(frame)/24.0
	...
	surface.baseline.free(); surface.candidate.free()
