...
for v in e:
 ...
 visible_outside=(~mask)&(original[:,:,3]>0); assert np.array_equal(original[visible_outside],out[visible_outside]) and np.array_equal(original[:,:,3][~mask],out[:,:,3][~mask]),v['id']
 if v['state']=='active':
  idle=np.array(Image.open(b/'composites'/(v['id'].replace('_active_','_idle_')+'.png')).convert('RGBA'));changed=int(np.any(idle!=out,axis=2).sum());assert changed>0;report.append({'id':v['id'],'status_pixels':changed,'visible_exterior_and_alpha_unchanged':True})
