...
func _objective_summary() -> String:
	...
	for objective in current_objectives:
		...
		parts.append(
			"%s %s %s" % [
				prefix,
				str(objective.get("label", objective.get("id", "OBJ"))).to_upper(),
				ObjectiveRules.progress_text(objective, objective_progress)
			]
		)
	return "   ".join(parts)
...
func _draw_title() -> void:
	...
	draw_string(
		ThemeDB.fallback_font,
		Vector2(0, 207),
		"ENTER LAUNCH   U WEAPON   G GENERATOR   H/J SERVICE",
		HORIZONTAL_ALIGNMENT_CENTER,
		640,
		11,
		Color("d8dde2")
	...
	draw_string(
		...
		Vector2(0, 236),
		"%s   %s   CREDITS %06d" % [
			str(weapon.get("name", "CANNON")).to_upper(),
			str(generator.get("name", "GENERATOR")).to_upper(),
			credits
		],
		HORIZONTAL_ALIGNMENT_CENTER,
		640,
...
func _draw_result() -> void:
	...
	draw_string(
		ThemeDB.fallback_font,
		Vector2(0, 146),
		"SCORE %08d   CREDITS %06d" % [score, credits],
		HORIZONTAL_ALIGNMENT_CENTER,
		640,
		14,
		Color("e3e6e8")
...
func _draw_gameplay() -> void:
	...
	draw_string(
		ThemeDB.fallback_font,
		Vector2(16, 26),
		"H%03d S%03d E%03d B%01d W%02d T%03d %08d" % [
			hull,
			shield,
			energy_pct,
			bombs,
	...
	draw_string(
		...
		"%s  %s  %s" % [
			current_mission_name,
			str(_active_weapon().get("name", "CANNON")).to_upper(),
			str(_active_generator().get("name", "GENERATOR")).to_upper()
		],
		HORIZONTAL_ALIGNMENT_LEFT,
		-1,
		9,
