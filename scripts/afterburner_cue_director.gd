...
const PRIMARY_RING_SIZE := Vector2(64,34)
const PRIMARY_RING_END_SIZE := Vector2(268,138)
const SECONDARY_RING_SIZE := Vector2(42,22)
const SECONDARY_RING_END_SIZE := Vector2(188,94)
const PRESSURE_FRONT_TRAVEL := 66.0
const ENGINE_MOUNTS := {
...
func draw_afterburner(surface: CanvasItem) -> void:
	...
	if _boom_age < SONIC_BOOM_DURATION and _has_property(scene, "player_position"):
		...
		var ring_center := p + Vector2(0,29.0+PRESSURE_FRONT_TRAVEL*ease(t,0.78))
		var ring_ease := ease(t,0.72)
		var ring_size := PRIMARY_RING_SIZE.lerp(PRIMARY_RING_END_SIZE,ring_ease).round()
		var ring_alpha := 0.90*(1.0-smoothstep(0.52,0.90,t))*flash_scale
		...
		if _boom_age >= SECONDARY_RING_DELAY:
			...
			var wake_center := p+Vector2(0,31.0+PRESSURE_FRONT_TRAVEL*0.82*ease(wake_t,0.78))
			var wake_alpha := 0.46*(1.0-smoothstep(0.46,0.84,wake_t))*flash_scale
