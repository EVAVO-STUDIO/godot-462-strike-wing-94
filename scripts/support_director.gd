...
func _fire_projectiles(scene: Object, support: Dictionary, homing: bool) -> void:
	...
	for i in range(angles.size()):
		var bullet := {
			...
			"velocity": Vector2.UP.rotated(float(angles[i])) * speed,
			"damage": damage,
			"support": true,
	...
	for i in range(bullets.size()):
		...
		if target != null:
			...
			if desired.length_squared() > 0.001 and velocity.length_squared() > 0.001:
				var max_turn := float(bullet.get("turn_rate", 2.4)) * delta
				var turn := clampf(velocity.normalized().angle_to(desired), -max_turn, max_turn)
