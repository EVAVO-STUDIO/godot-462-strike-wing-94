...
The aircraft advances without a longitudinal screen boundary. Its route coordinate is unbounded; the camera follows that coordinate with a smooth speed-dependent look-ahead. Terrain uses camera distance, while encounters and arrival gates use aircraft distance. Existing contact closure already includes forward speed, so retained projectiles, contacts, combat effects and strike markers receive only the additional camera projection adjustment. Airborne closure now also decreases below cruise. At the lateral display limits, directional recovery chevrons and a four-second mission-airspace countdown replace the unexplained hard-stop behavior; inward steering clears the countdown. Wider authored environment margins and terrain collision corridors can extend this projection in a later map-production pass.
