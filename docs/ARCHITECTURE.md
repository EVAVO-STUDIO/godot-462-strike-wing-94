...
`BossRules` also owns silhouette-registered elliptical projectile and expanded craft-contact geometry for every command target. The extents follow each reviewed opaque fighting mass: transparent canvas corners, antennae and effect padding remain non-colliding, while rounds and the VX-94 cannot pass through visible wings, hulls or railcars. Boss contact computes the minimum ray-to-ellipse separation plus a four-pixel air gap, clamps the displaced fighter back into its flight envelope, and uses a 0.55-second recovery gate. A ram therefore reads as one severe physical impact rather than damage applied every render frame.
...
`CampaignSave` is canonical schema **v12**. It persists stable mission IDs so authored campaign reordering and optional branches do not reinterpret progress; v1-v11 saves remain migration-compatible, including the original 12-mission index map. Secret-operation clear history is stored independently from the 30-sortie campaign position, and sanitized career statistics retain long-term sorties, accuracy, destruction, damage, discoveries, rewards, and best-score performance.
...
- unlocked tactical-intelligence file IDs.
...
`tools/validate.ps1` performs local structural/data/save checks without network or paid CI. `tools/validate_windows_release.ps1` is the stronger shipment gate: it runs that complete suite, proves stable 60 Hz against the native production-combat stress workload, captures a twenty-six-state 640×360 matrix spanning the approved EVAVO/title sequence, complete front-end/pause/credits vocabulary, both halves of the flight-control catalogue, compact routine objective HUD, and representative combat for human review, runs bounded live telemetry through eight campaign/secret-gameplay representatives, builds the embedded-PCK Windows executable, verifies EVAVO/HYPERSONIC metadata, and smoke-launches the packaged runtime.

`tools/playtest_telemetry_probe.gd` attaches only for the explicit `--playtest-telemetry` launch profile. It advances the real gameplay scene with deterministic input, samples form and altitude occupancy, combat activity and live director acceptance, then writes ignored JSON reports under `work/playtest_telemetry`. The matrix covers the first, bomber-heavy, difficult-air, altitude-choice, machine-reveal, orbital-transition, secret and final sorties. This is a runtime-integration and suspicious-metrics gate; it does not claim to automate human judgment of fun, pacing, readability or sound mix.
