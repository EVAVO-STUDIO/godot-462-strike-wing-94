...
The release gate first runs the complete source/engine suite and native 1280×720 production-combat stress profile, then the canonical `Windows Desktop` preset produces a single embedded-PCK executable at `build/windows/HYPERSONIC.exe`. The performance gate sustains 14 enemies, 16 player projectiles and 64 hostile projectiles with all production layers active, requiring at least 60 average FPS and a 16.67 ms-or-better p95 frame time. Source-production art, documentation, tools and local work captures are excluded from the player package. The export and verification scripts refuse paths outside the repository's ignored `build` directory. Verification checks Windows identity metadata and launches the packaged game through a deterministic headless front-door smoke test. `-SkipPerformance` exists only for non-graphical packaging diagnosis; such a run is not a complete release-performance audit. The lower-level profile, export and verification commands remain available for focused diagnosis.
